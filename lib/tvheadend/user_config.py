...
class TVHUserConfig(lib.user_config.UserConfig):
    ...
    def __init__(self, script_dir=None, opersystem=None, args=None, config=None):
        ...
        self.data['main'].update({
            ...
            'config_file': None,
            'web_admin_port': '6077',  # port for non-streaming requests
        })
        self.data.update({
            'freeaccount': {
        ...
        self.data.update({
            'epg': {
                ...
                'min_refresh_rate': 14400,  # 4 hours based on web-based requests
            }})
        self.data.update({
            'player': {
                'stream_type': None,  # m3u8redirect, ffmpegproxy, internalproxy
                'ffprobe_path': None,
                'stream_filter': None,  # regexp to filter the stream files when direct streaming
            }})
        self.data.update({
            'hdhomerun': {
                'disable_hdhr': True,  # used to support HDHomerun auto-discovery.
                'hdhr_id': None,  # This is a 4 byte with CRC_4 hex string.  EX: 104ed44c
        ...
        for (key, value) in updated_data[section].items():
            if value[1]:
                if value[0] is None:
                    ...
                    self.logger.debug('Config Update: Removed [{}][{}]'.format(section, key))
                    ...
                    self.logger.debug('Config Update: Changed [{}][{}] to {}'
    ...
    def tvh_config_adjustments(self, opersystem, script_dir):
        ...
        self.data['main']['reporting_friendly_name'] = "Locast Rocky4546"
        self.data['main']['reporting_model'] = "HDHR5-4US"
        self.data['main']['reporting_firmware_name'] = "hdhomerun5_atsc"
        ...
        if self.data['hdhomerun']['hdhr_id'] is None:
            self.data['hdhomerun']['hdhr_id'] = self.hdhr_gen_device_id()
            self.write('hdhomerun', 'hdhr_id', self.data["hdhomerun"]['hdhr_id'])
        else:
            if not self.hdhr_validate_device_id(self.data['hdhomerun']['hdhr_id']):
                self.data['hdhomerun']['hdhr_id'] = self.hdhr_gen_device_id()
                self.write('hdhomerun', 'hdhr_id', self.data["hdhomerun"]['hdhr_id'])
        ...
        if type(self.data['epg']['min_refresh_rate']) is not int:
            self.data['epg']['min_refresh_rate'] = int(self.data['epg']['min_refresh_rate'])
        ...
        if type(self.data['main']['disable_ssdp']) is str:
        ...
        if CRYPTO_LOADED and self.data['main']['encrypt_key'] is None \
            ...
            if self.data['main']['locast_password'].startswith(ENCRYPT_STRING):
                ...
                if self.data['main']['locast_password'] == 'UNKNOWN':
                    self.logger.error('Unable to decrypt password. '
