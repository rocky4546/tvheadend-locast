...
def hdhr_process(config, tuner_queue):
    utils.logging_setup(config['main']['config_file'])
    logger = logging.getLogger(__name__)

    hdhr = HDHRServer(config, tuner_queue)
...
class HDHRServer:
    ...
    def process_client_connection(self, connection, address):
        ...
        self.logger.debug('TCP: New connection established {}'.format(address))
        try:
            while True:
                msg = connection.recv(1316)
                if not msg:
                    # client disconnect
                    self.logger.debug('TCP: Client terminated connection {}'.format(address))
                    break
                ...
                if frame_type == HDHOMERUN_TYPE_GETSET_REQ:
                    ...
                    self.logger.error('TCP: Unknown frame/message type from {} type={}'.format(address, frame_type))
        finally:
            connection.close()
    ...
    def gen_err_response(self, frame_type, tag, text):
        # This is a tag type of HDHOMERUN_ERROR_MESSAGE
        # does not include the crc
        msg = msgs[tag].format(*text).encode()
    ...
    def create_getset_response(self, req_dict, address):
        ...
        if name == b'/sys/model':
            ...
            if name_str.endswith('/lockkey'):
                self.logger.error('TCP: NOT IMPLEMENTED GETSET LOCKKEY MSG REQUEST: {} '.format(req_dict))
                ...
                self.logger.error('TCP: NOT IMPLEMENTED GETSET MSG REQUEST: {} '.format(req_dict))
                return None
        else:
            self.logger.error('TCP: 3 UNKNOWN GETSET MSG REQUEST: {} '.format(req_dict))
            return None
    ...
    def parse_getset_request(self, msg):
        ...
        if crc_calc == crc_rcvd:
            ...
            request_info = {}
            while True:
                (msg_type, value, offset) = self.get_id_value(msg, offset)
                if msg_type is None:
                    break
    ...
    def get_id_value(self, msg, offset):
        ...
        (msg_type, length) = struct.unpack('BB', msg[offset:offset + 2])
        offset += 2
        (value,) = struct.unpack('%ds' % (length - 1), msg[offset:offset + length - 1])
        offset += length
        return msg_type, value, offset

    def run_multicast(self, bind_ip=''):
    ...
    def datagram_received(self, data, host_port):
        ...
        if is_allowed:
            ...
            if frame_type != HDHOMERUN_TYPE_DISCOVER_REQ:
                ...
                crc = struct.pack('<I', x)
                response += crc
                self.logger.debug('UDP Response={} {}'.format(host_port, response))
