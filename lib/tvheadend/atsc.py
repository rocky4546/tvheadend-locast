...
from lib.tvheadend.algorithms import Crc
from lib.tvheadend.models import CrcModels
...
class ATSCMsg():
    ...
    def gen_crc_mpeg(self, msg):
        ...
        crc_int = alg.bit_by_bit(msg)
        crc = struct.pack('>I', crc_int)
        return crc


    def gen_header(self, pid):
        # pid is an integer
        # pid = PMT channel pid
        #       STT 1FFB
        #       VCT 1FFB
        ...
        pid_shifted = pid << 8
        msg_int = sync | pid_shifted |  (self.msg_counter[pid] + 16)   # x10
    ...
    def gen_multiple_string_structure(self, names):
    ...
    def gen_pid(self, prog_number):
        ...
        return (pid_lookup[prog_number])
        
    def gen_lang(self, name):
        return struct.pack('%ds' % (len(name)), name)
        
    def update_sdt_names(self, sdt_msg, service_provider, service_name):
        ...
        # crc
        if sdt_msg[:3] != b'\x47\x40\x11':
            self.logger.info('WRONG ATSC MSG {}'.format(bytes(sdt_msg[:20]).hex()))
            return sdt_msg

        descr = b'\x01' \
            + utils.set_str(service_provider, False) \
            + utils.set_str(service_name, False)
        descr = b'\x48' + utils.set_u8(len(descr)) + descr
        msg = sdt_msg[8:20] + utils.set_u8(len(descr)) + descr
        length = utils.set_u16(len(msg) + 4 + 0xF000)  
        msg = ATSC_SERVICE_DESCR_TABLE_TAG + length + msg
        crc = self.gen_crc_mpeg(msg)
        msg = sdt_msg[:5] + msg + crc
        msg = msg.ljust(len(sdt_msg), b'\xFF')
        return msg
        
        
        
    def gen_sld(self, base_pid, elements):
        ...
        elem_len = utils.set_u8(len(elements)+1)
        video_pid = utils.set_u16(base_pid + 1 + 57344)  # E000
        stream_type = b'\x02'
        lang_0 = b'\x00\x00\x00'
        msg = stream_type + video_pid + lang_0
        stream_type = b'\x81'
        audio_pid_int = base_pid + 3 + 57344  # E000 (starts at 0x34 and increments by 1)
    ...
    def gen_vct_channel(self, tsid, short_name, channel):
        ...
        # long_name
        u16name = b''
        short_name7 = short_name.ljust(7, '\x00')
        
        for ch in short_name7:
            u16name += utils.set_u16(ord(ch))
        ch_num = channel['chnum_maj'] << 10
        ch_num |= 15728640   # 0xf00000 static bits
        ch_num |= channel['chnum_min']
        u3bch_num = utils.set_u32(ch_num)[1:]
        mod_mode = b'\x04'
        freq = b'\x00\x00\x00\x00'
        prog_num = utils.set_u16(channel['prog_num'])
        pid = self.gen_pid(channel['prog_num'])
        ...
        return u16name + u3bch_num + mod_mode + freq + tsid + prog_num + misc_bits + \
            source_id + descr_len + descr_msg


    def gen_pat_channels(self, channels):
    ...
    def gen_pat(self, mux_stream):
        ...
        #gen_pat_channels()
        #crc
        tsid = mux_stream['tsid']
        ver_sect = b'\xc3\x00\x00'
        channels_len = utils.set_u8(len(mux_stream['channels']))
        for i in range(len(mux_stream['channels'])):
            pid = self.gen_pid(i)
        msg = tsid + ver_sect + self.gen_pat_channels(mux_stream['channels'])
    ...
    def gen_vct(self, mux_stream):
        ...
        channels_len = utils.set_u8(len(mux_stream['channels']))
        for short_name in mux_stream['channels'].keys():
            msg += self.gen_vct_channel(tsid, short_name, mux_stream['channels'][short_name])
    ...
    def gen_pmt(self, channels):
        ...
        msgs=[]
        prog_num_int = 0
        for short_name in channels.keys():
            prog_num_int += 1
    ...
    def format_video_packets(self, msgs):
        ...
        if len(msgs) > 7:
            self.logger.error('ATSC: TOO MANY MESSAGES={}'.format(len(msgs)))
            return None
        for i in range(len(msgs)):
            if len(msgs[i]) > 188:
                self.logger.error('ATSC: MESSAGE LENGTH TOO LONG={}'.format(len(msg)))
                return None
            else:
                sections[i] = msgs[i].ljust(188, b'\xff')
